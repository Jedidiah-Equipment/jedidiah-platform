...
- The former hard requirement "Paint schedules only Paint Bays" is formally dropped, not just unimplemented. `job:schedule` now means scheduling every Bay; only `admin` holds it, so all schedule mutation (Slots, Bay Calendar Exceptions, org Off-Days) is admin-only for now.
