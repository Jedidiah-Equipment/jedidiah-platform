...
**Quote Line Item** is a freeform Product Quote charge with a name, positive whole-number quantity, and non-negative unit price. It has no catalog semantics and always contributes quantity × unit price to Quote Pricing. Custom Quotes do not carry Line Items — they carry Work Items instead.

**Work Item** is a Custom Quote charge describing one unit of custom work: a required name, a labour duration in hours (fractional, zero allowed), and zero or more Work Item Parts. Its total is duration × the Quote's Hourly Rate plus the sum of its Parts, rounded to cents; it contributes that total to Quote Pricing exactly where Line Items sit on Product Quotes (inside the discount, before VAT). Labour-only, parts-only, and zero-cost Work Items are all legal, as is a Custom Quote with no Work Items. Work Items are commercial facts and freeze under Quote locking. The Custom Job displays its Quote's Work Items by name only; the customer-facing Quote Document prints each Work Item as name + total, never the Hourly Rate, hours, or part breakdown.
