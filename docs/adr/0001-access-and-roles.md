...
- `procurement-manager`: full Purchase Order access except approval; inventory read/adjust and cost read/revalue, but no general inventory move; Customer directory and Product, Part, and Supplier management; Quote create/read/update, without `quote:cancel`; Product Unit and Job reads; no Bay scheduling mutation.
- `job-manager`: Job updates, including fabrication timing stamps, plus Job and Product Unit reads; no Job creation, scheduling, calendar, or cancellation mutation.
...
Inventory v1 adds the `purchase_order` (read/create/send/amend/receive/close), `inventory` (read/move/adjust/count/build/close-out), and `inventory_cost` (read/revalue) resources. Admins receive every new permission, while sales, job-manager, job-viewer, and bay-operator receive none. A single server-side projection rule nulls cost fields for callers without `inventory_cost:read`. Granting `purchase_order:receive` always implies authority to post the corresponding receipt movements; receiving is not a paper-only action.
