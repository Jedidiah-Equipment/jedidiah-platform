...
// A `cover` slot's recommendation is its brochure frame's ratio, so nothing is cropped away. The
// `contain` technical drawing is never cropped, so its recommendation is the wide landscape the
// drawings themselves are produced at rather than the narrower frame they are scaled into.
