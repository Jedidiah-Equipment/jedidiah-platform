...
describe('canStartJobFromQuote', () => {
  it('rejects quotes that already have a job', () => {
    expect(canStartJobFromQuote({ hasJob: true, hasProductUnit: false, kind: 'custom', status: 'draft' })).toEqual({
      allowed: false,
  ...
  it.each(['draft', 'sent', 'accepted'] as const)('allows custom %s quotes to start a job', (status) => {
    expect(canStartJobFromQuote({ hasJob: false, hasProductUnit: false, kind: 'custom', status })).toEqual({
      allowed: true,
    });
  });
  ...
  it('allows accepted product quotes to start a job', () => {
    expect(canStartJobFromQuote({ hasJob: false, hasProductUnit: false, kind: 'product', status: 'accepted' })).toEqual(
      { allowed: true },
    );
  });
  ...
  it.each(['draft', 'sent', 'rejected', 'cancelled'] as const)('rejects product %s quotes', (status) => {
    expect(canStartJobFromQuote({ hasJob: false, hasProductUnit: false, kind: 'product', status })).toEqual({
      allowed: false,
