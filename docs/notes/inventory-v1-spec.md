...
- **Sent POs take logged amendments** — quantity change (either direction), add line, substitute part, or expected-delivery-date change; who/when/note; PDF regenerable. The phone call that changes an order is the recorded event. No auto-generated supplementary POs.
...
- **Fabricated sheet-metal parts carry zero material cost** — their material is charged through the raw-material lines; costing them separately would pay for the plate twice. *(Narrowed at implementation, #1058: this is a consequence, not a stored rule. A plate-cut Part's ledger simply holds no costed rows, so its average is null — "no cost yet", which §5 already requires — while a Built Part assembled from **stocked** components carries the value its build moved onto it. Hardcoding zero would have made that value vanish at the next draw instead of reaching the Job. Hand-entered costs on a built Part remain refused; its cost comes from its build.)*
