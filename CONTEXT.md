...
**Quote** is a sales offer for one **Customer**, owned by one salesperson. A **Product Quote** references one Product, can source at most one Job once accepted, and becomes a **Locked Quote** once it sources a Job: commercial facts stay frozen, while post-sale logistics and notes can still change where the UI allows. A **Custom Quote** has no Product; it uses a Work Title, an entered base price, an **Hourly Rate** (seeded from an org-wide default, editable per Quote), and Work Items; it can source at most one Custom Job while draft, sent, or accepted, and locks commercial facts once accepted except that its Hourly Rate and Work Items remain editable. A Quote can be **Cancelled** into a terminal, irreversible status that also locks it: an unlocked Quote via the status dropdown (`quote:update`), a Locked Quote via an admin-only destructive flow (`quote:cancel`). Cancelling a Quote that has sourced a Job cascades to **cancel that Job** and remove its future (`scheduled`) Slots in one transaction. The remedy for a mistaken cancellation is a new Quote.
...
**Work Item** is a Custom Quote charge describing one unit of custom work: a required name, a labour duration in hours (fractional, zero allowed), and zero or more Work Item Parts. Its total is duration × the Quote's Hourly Rate plus the sum of its Parts, rounded to cents; it contributes to the discountable ex-VAT subtotal. Labour-only, parts-only, and zero-cost Work Items are all legal, as is a Custom Quote with no Work Items. Work Items and the Hourly Rate are explicit commercial-fact exceptions to Quote locking and remain editable after acceptance and after a Custom Job starts. The Custom Job displays its Quote's Work Items by name only; the customer-facing Quote Document prints each Work Item as name + total with a positive-duration Labour row (hours + hourly rate + subtotal) followed by its Parts (quantity + name + unit price + subtotal). Zero-hour Work Items omit the Labour row.
