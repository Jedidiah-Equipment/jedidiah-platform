...
const LINE_B = 'line-b';
const lines = [
  { id: LINE_A, quantity: 10 },
  { id: LINE_B, quantity: 4 },
];

describe('derivePurchaseOrderProgress', () => {
  it('reads an order nobody has receipted as still just sent', () => {
    expect(derivePurchaseOrderProgress({ lines, receivedByLineId: new Map() })).toBe('sent');
  });

  it('reads a first partial delivery as partially received', () => {
    expect(derivePurchaseOrderProgress({ lines, receivedByLineId: new Map([[LINE_A, 3]]) })).toBe('partially-received');
  });
  ...
  it('holds at partially received while any line is still short', () => {
    ...
    expect(derivePurchaseOrderProgress({ lines, receivedByLineId })).toBe('partially-received');
  });
  ...
  it('reads exactly the ordered quantity on every line as received', () => {
    ...
    expect(derivePurchaseOrderProgress({ lines, receivedByLineId })).toBe('received');
  });
...
describe('derivePurchaseOrderStatus', () => {
  it('passes a draft through untouched — receipts cannot exist before it is sent', () => {
    expect(
      derivePurchaseOrderStatus({ closedShortAt: null, lines, receivedByLineId: new Map(), status: 'draft' }),
    ).toBe('draft');
  });

  it('passes an approved order through untouched — it has not gone out to be received against', () => {
    expect(
      derivePurchaseOrderStatus({ closedShortAt: null, lines, receivedByLineId: new Map(), status: 'approved' }),
    ).toBe('approved');
  });

  it('reads a sent order nothing has arrived against as approved — the Sent tick carries the rest', () => {
    expect(derivePurchaseOrderStatus({ closedShortAt: null, lines, receivedByLineId: new Map(), status: 'sent' })).toBe(
      'approved',
    );
  });
  ...
  it('passes a cancelled order through untouched', () => {
    expect(
      derivePurchaseOrderStatus({ closedShortAt: null, lines, receivedByLineId: new Map(), status: 'cancelled' }),
    ).toBe('cancelled');
  });
  ...
  it('projects a sent order through its receipts', () => {
    expect(
      derivePurchaseOrderStatus({
        closedShortAt: null,
        lines,
        receivedByLineId: new Map([[LINE_A, 3]]),
        status: 'sent',
      }),
