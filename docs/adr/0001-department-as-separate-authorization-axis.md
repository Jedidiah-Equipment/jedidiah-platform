# Job authorization: roles grant verbs, departments scope rows

Job scheduling access is governed by **two axes**: the flat `AppRole` enum grants verb capabilities (`job:read`, `job:schedule`, …), and `user_department` membership scopes which Department Bays a Department-Aware role can schedule. A User with one or more Department memberships is scoped to those Departments. A User with no selected Departments is explicitly unscoped and can schedule every Bay their role has permission for. The Users UI must warn loudly: **If no departments are selected, the user can schedule ALL Bays.**

The Department-Aware roles are `job-supervisor` and `job-department-manager`.

## Considered Options

- **Single axis — encode departments as roles.** Rejected: would explode the role enum combinatorially as Departments grow and conflate two unrelated concerns (capability vs. scope).
- **Single axis — flat job editor/viewer roles with no Department membership at all.** Rejected: cannot express "Paint schedules only Paint Bays", which is a hard requirement.
- **Two axes, with "empty department set = unscoped".** Accepted: this keeps Department membership as the only row-scope control and avoids a second role-scope matrix. The footgun is handled operationally by making the empty-selection meaning explicit in the Users UI.
- **Two axes, scope encoded as a `scope:` marker on each role.** Rejected: premature abstraction — the policy module can name the Department-Aware roles directly. Revisit only if a third authorization axis appears.

## Consequences

- `appRoleAccess` still answers only "which verbs"; the Job Authorization Policy answers "which rows."
- Empty Department membership is not "no access." It means "all Departments" for any Bay scheduling permission the role already has.
- Any UI that edits Department membership must surface the empty-selection rule prominently before saving can surprise an operator.
