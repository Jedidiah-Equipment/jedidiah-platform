...
Documents carry descriptive **metadata** whose required shape differs by owner type. Rather than typed columns per field, metadata lives in a single `metadata` jsonb column on `documents`, validated by a per-owner-type Zod schema co-located with `DocumentPolicy` in `pkg/domain` and enforced on write in `pkg/core`. The first instance: a Product's Documents must declare a `type` of `sop | part_book | brochure`, used only to group documents for display.
...
- **`type` is a label only.** Nothing branches on it — not permissions, not `DocumentPolicy` content/size rules, not snapshot selection. It exists to group a Product's documents ("Part Books", "SOPs", "Brochures") in the UI.
...
- It keeps the line we drew during the provenance discussion (ADR 0030): **relational / queryable / enforced-as-a-relationship → column or FK; descriptive / rendered-only → `metadata`.** Provenance (`source_product_id`) is a real FK and stayed a column; `type` is a descriptive label grouped over a handful of rows client-side, so it belongs in metadata. Provenance must **never** live in `metadata`.
...
- Grouping by `type` is client-side over a small per-owner list, not SQL aggregation across owners. If cross-owner reporting on metadata ever becomes a need (as part demand did for the CFO in ADR 0028), that is the trigger to revisit — likely an expression index on `metadata->>'…'` before any column promotion.
