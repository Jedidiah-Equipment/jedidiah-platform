...
export const purchaseOrdersRouter = router({
   ...
   */
  amendQuantity: authorizedProcedure('equipment_purchase_order:amend')
  ...
  amendExpectedDate: authorizedProcedure('equipment_purchase_order:amend')
  ...
  amendSubstitutePart: authorizedProcedure('equipment_purchase_order:amend')
  ...
  amendments: authorizedProcedure('equipment_purchase_order:read')
  ...
  documents: authorizedProcedure('equipment_purchase_order:read')
    .input(PurchaseOrderCollectionInput)
   ...
   *
   * Gated on `equipment_inventory_cost:read` as a whole rather than field by field: the panel is prices — what
   ...
   */
  supplierInvoices: authorizedProcedure('equipment_inventory_cost:read')
    .input(PurchaseOrderCollectionInput)
   ...
   *
   * Double-gated: `equipment_inventory_cost:revalue` is what the revaluation itself needs, and the read gate
   * is asserted beside it because confirming a price you were never shown is not a confirmation.
   */
  applyInvoicePrice: authorizedProcedure('equipment_inventory_cost:revalue')
  ...
  dismissInvoiceFlag: authorizedProcedure('equipment_inventory_cost:read')
  ...
  invoicePriceVariance: authorizedProcedure('equipment_inventory_cost:read')
   ...
   */
  returnToSupplier: authorizedProcedure(['equipment_inventory:move', 'equipment_purchase_order:amend'])
   ...
   */
  partLines: authorizedProcedure('equipment_purchase_order:read')
  ...
  returns: authorizedProcedure('equipment_purchase_order:read')
    .input(PurchaseOrderCollectionInput)
  ...
  returnsAwaitingCredit: authorizedProcedure('equipment_purchase_order:read')
  ...
  cancel: authorizedProcedure('equipment_purchase_order:close')
  ...
  closeShort: authorizedProcedure('equipment_purchase_order:close')
    .input(PurchaseOrderActionInput)
    .output(PurchaseOrderView)
    .mutation(async ({ ctx, input }) => {
      const purchaseOrder = await mapPurchaseOrderErrors(() =>
  ...
  create: authorizedProcedure('equipment_purchase_order:create')
  ...
  get: authorizedProcedure('equipment_purchase_order:read')
    .input(PurchaseOrderActionInput)
    .output(PurchaseOrderView)
    .query(async ({ ctx, input }) => {
  ...
  list: authorizedProcedure('equipment_purchase_order:read')
   ...
   * `equipment_purchase_order:read` rather than `equipment_inventory:read`: every row is a Purchase Order, named by its
   * code and its Supplier, and it is the PO screens it sends the reader to.
   */
  late: authorizedProcedure('equipment_purchase_order:read')
   ...
   */
  approve: authorizedProcedure('equipment_purchase_order:approve')
    .input(PurchaseOrderActionInput)
    .output(PurchaseOrderView)
    .mutation(async ({ ctx, input }) => {
      const purchaseOrder = await mapPurchaseOrderErrors(() =>
  ...
  markSent: authorizedProcedure('equipment_purchase_order:send')
    .input(PurchaseOrderActionInput)
    .output(PurchaseOrderView)
    .mutation(async ({ ctx, input }) => {
      const purchaseOrder = await mapPurchaseOrderErrors(() =>
  ...
  revertToDraft: authorizedProcedure('equipment_purchase_order:approve')
    .input(PurchaseOrderActionInput)
    .output(PurchaseOrderView)
    .mutation(async ({ ctx, input }) => {
      const purchaseOrder = await mapPurchaseOrderErrors(() =>
   ...
   */
  receive: authorizedProcedure('equipment_purchase_order:receive')
  ...
  /**
   * A ticked selection becomes one draft per Supplier. Gated on `equipment_purchase_order:create` alone: its
   ...
   */
  createFromSelection: authorizedProcedure('equipment_purchase_order:create')
  ...
  saveDraft: authorizedProcedure('equipment_purchase_order:create')
