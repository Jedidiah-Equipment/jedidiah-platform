...
**Slot Projection**:
The read-time computation that turns a Bay's queue of Slots into concrete calendar dates. It walks the Bay's Slots in queue order from the Bay's fixed anchor, consuming each Slot's `durationDays` as **working days** and skipping the Bay's Off-Days per the Working Calendar: the first Slot starts on the anchor's first working day, each later Slot starts on the next working day after the previous one ends, and a Slot's working days may straddle Off-Days. Projected dates are whole `yyyy-MM-dd` **plant business dates** (ADR-0043), not instants — they read identically from any viewer timezone, and the projection's date arithmetic is timezone-free. Africa/Johannesburg exists only at the server boundary, where wall-clock now becomes plant "today" and a new Bay's `scheduleOrigin` anchor; `scheduleOrigin` itself is stored as a plain business date. Dates are never stored on a Slot — only sequence, kind, duration, and Slot-specific references/labels are. Because dates are derived, resizing or removing a Slot, or changing which dates are Off-Days, naturally reflows everything after it. Idle time inside the queue is represented by Idle Slots, never by moving `scheduleOrigin` or silently flooring the whole projection to today; Off-Days (weekends, holidays, shutdowns) are not Idle Slots — they are non-working dates the projection steps over.
...
**Job Bay Seeding**:
The optional Bay-assignment step of starting a Job, hosted on the dedicated Start Job page reached from an accepted Quote (it outgrew a dialog once it embedded the live Gantt). The form pre-fills editable rows from the Quote Product's enabled Product Bays, silently skipping Product Bays whose Bay is disabled; Products with no enabled Product Bays start with zero selected rows. It lists every enabled Bay grouped by Department in Pipeline order and lets the planner add or remove Bays (including several in one Department) and edit each row's working-days before submitting. Each row also carries that Bay's start date, defaulting to max(next available day, tomorrow); a holder of `job:schedule` may move it within the Insert at Date picker bounds (a date that will split an existing Slot shows a warning naming the affected Job or Idle label and the resulting durations). By design `job:create` implies `job:schedule` — creating a Job inherently schedules its seeds — so every creator holds scheduling authority; the read-only/append path exists only as a degraded fallback when schedule data is unavailable. Each retained row books one Work Slot into that Bay's queue via Insert at Date under the Bay's lock, in the same transaction as Job creation — a Job is never half-seeded — with `durationDays` counting working days, the same auto-inserted Idle Slot gap as Gantt booking when a Bay's queue ended before today, and the same clamp-to-append when a Bay's queue shrank below a picked date. The page embeds the live Bay schedule Gantt filtered to the seeded Bays in Pipeline order, rendering each seed as a client-only ghost Slot at its resolved Insert at Date placement — a date inside an existing Slot shows that Slot as its two halves around the ghost — recomputing as dates, durations, rows, or the underlying queues change; schedule edits made on the embedded Gantt are the same immediate live mutations as the Jobs page and survive Cancel (leaving the page), which discards only the uncreated Job and its ghosts. Seeding is optional: a Job may be created with zero Bays and scheduled entirely later on the Gantt.
