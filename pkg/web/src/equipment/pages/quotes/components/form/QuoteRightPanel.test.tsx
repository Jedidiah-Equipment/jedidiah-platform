...
test('heads a Parts Sale by its type and offers no Start Job', () => {
  const quote = {
    code: 34,
    customerCompanyName: 'Parts Counter Customer',
  ...
  expect(html).toContain('>Parts Sale<');
  expect(html).not.toContain(`>${quoteKindLabels.custom}<`);
  expect(html).not.toContain('data-start-job-link');
});
