...
A **Stock Movement** is one append-only, actor-attributed ledger fact that changes a Part's stock or cost; it is never edited or deleted. Stock on hand is the sum of movement deltas per Part, bucketed by length for linear Parts. A **Checkout** draws a Part from stock against a Job at the current moving average; a **Return to Store** reverses it at the cost still drawn by that Job, and stays available after the Job is cancelled so recovered stock is not stranded off-ledger. A **Receipt** is the arrival of stock against one open Purchase Order line — it attaches to that line, defaults its cost from the line's price and its length from the Part's standard purchase length, and is the ledger write receiving *is*, never a paper step posted later. Both Job movements are valid off-CFO, and all three warn rather than block when quantities disagree: a draw past the CFO, a draw taking a length bucket negative, a return past what the Job still holds, and a receipt past what the line ordered are all warnings, judged once and shown both before the post and on it. **Commitment** is derived per Job and Part as `max(0, CFO quantity − drawn quantity)` until inventory close-out; **Free Stock** is stock on hand minus the sum of open commitments.

**Stock Tracking Mode** is per Part: perpetual records every stock movement, while periodic records only its opening balance and stocktake counts — a periodic Part rejects consumption movements and every other adjustment reason outright, because between counts its stock on hand is deliberately stale-high and only a count can correct it.
