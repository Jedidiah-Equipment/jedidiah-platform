...
Job access is governed by **two axes**: the flat `AppRole` enum grants verb capabilities (`job:read`, `job-stage:update`, …), and `user_department` membership scopes *which rows* a Department-Aware role's verbs apply to. A User with one or more Department memberships is scoped to those Departments. A User with no selected Departments is explicitly unscoped and can see every Stage their role has permission for. The Users UI must warn loudly: **If no departments are selected, the user will see ALL stages.**
