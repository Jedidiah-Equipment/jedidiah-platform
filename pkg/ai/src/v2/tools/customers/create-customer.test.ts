...
function createContext(session = true): AiV2Context {
  return {
...
describe('createCustomer v2 contract', () => {
  test('normalizes Customer input, creates it as the actor, and returns linked details', async () => {
    ...
    expect(createCustomerDefinition.requiredPermission).toEqual(['customer:create']);
