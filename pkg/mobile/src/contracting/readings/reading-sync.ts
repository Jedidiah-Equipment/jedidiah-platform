...
import { type ReadingQueue, ReadingSyncError } from './reading-queue';
  ...
  await queue.sync(
    async (item) => {
      try {
        ...
        if (!(error instanceof ReadingSyncError) && !retryFailure && isActive()) {
          retryFailure =
            error instanceof Error ? error : new Error('Waiting to sync. Check your connection and sign-in.');
        }
        throw error;
      }
    },
