import { type Db, productRanges, products, sql } from '@pkg/db';
...
export async function resolveRangeImageRef(db: Db, rangeId: string): Promise<ResolvedImageRef | null> {
  ...
  const [row] = await db
    ...
    .from(productRanges)
    .where(sql`${productRanges.id} = ${parsed.data} and ${productRanges.deletedAt} is null`)
    .limit(1);
...
export async function resolveProductImageRef(
  ...
  const [row] = await db
    ...
    .from(products)
    .where(sql`${products.id} = ${parsed.data} and ${products.deletedAt} is null`)
    .limit(1);
