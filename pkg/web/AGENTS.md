...
- Route Equipment React Query invalidation through `src/equipment/hooks/use-query-invalidation.ts`; invalidate whole affected tRPC root paths. Shared auth cache clearing stays in `src/hooks/use-clear-query-cache.ts`, and the shared user admin invalidates through `src/hooks/use-user-admin-invalidation.ts`.
