...
- **Stage / Station Booking state remains derived from dates.** Stage / Station Booking status (`pending | in-progress | complete`) comes from the Actual Window. `stage.started/ended` and `job.started/completed` events are emitted at Station-write time when a rollup-compare shows the derived Actual Window flipping from absent to present. (Job *status* is no longer derived — it is a manually-set stored field; see [ADR-0021](./0021-job-status-is-a-manual-stored-decoupled-field.md).)
