import { formatBytes, IMAGE_ACCEPT } from '@pkg/domain';
...
export function validateSelectedBrochureImage(file: File | null): File | null {
  ...
  if (!ALLOWED_CONTENT_TYPES.has(file.type)) {
    toast.error('Only PNG or JPEG images can be uploaded.');
    return null;
  }

  if (file.size > BROCHURE_IMAGE_MAX_BYTES) {
    toast.error(`Image must be ${formatBytes(BROCHURE_IMAGE_MAX_BYTES)} or smaller.`);
    return null;
  }
