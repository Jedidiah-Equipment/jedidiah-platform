...
**Supplier** is currently a standalone procurement directory record. **Part** is the reusable purchasable item layer. A Part's Unit of Measure is its counting unit everywhere. A **Part Label** is an on-demand printable identity for exactly one Part: Code 128 encodes only the Part code, while readable text repeats its code, name, and Storage Location. Labels are never stored or tracked, and linear labels stay per Part rather than per length bucket. A **Stock Movement** is one append-only, actor-attributed ledger fact that changes a Part's stock or cost; it is never edited or deleted. Stock on hand is the sum of movement deltas per Part, bucketed by length for linear Parts. A **Checkout** draws a Part from stock against a Job at the current moving average; a **Return to Store** reverses it at the cost still drawn by that Job. Both are valid off-CFO and warn rather than block when quantities disagree. **Commitment** is derived per Job and Part as `max(0, CFO quantity − drawn quantity)` until inventory close-out; **Free Stock** is stock on hand minus the sum of open commitments. A Part's moving average cost is derived from ordered cost-bearing movements, is per millimetre for linear Parts, and remains absent — “no cost yet,” never zero — until a cost is established. Internally fabricated Parts are the deliberate exception: they carry zero material cost because their raw material is charged separately. **Stock Tracking Mode** is per Part: perpetual records every stock movement, while periodic records receipts and stocktake counts but no consumption movements. **Storage Location** is findability-only, never a stock-ledger dimension. A linear Part uses `mm` and must have a positive **Standard Purchase Length** in millimetres. **Assembly** is a Product-owned grouping of Parts, either Standard or Optional; an Assembly's kind is immutable after creation. Optional Assemblies carry an upgrade-delta price, may replace whole Standard Assemblies, and are selected on Product Quotes.
