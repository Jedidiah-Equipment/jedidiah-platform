...
**Idle Slot**:
A Slot that intentionally represents downtime in a Bay queue — a working day deliberately left empty, distinct from an Off-Day, which is a non-working date stepped over. It has no Job, advances Slot Projection exactly like a Work Slot (its `durationDays` counts working days and skips Off-Days), and is rendered differently from booked Job work on the schedule chart. Idle Slots may have a nullable label; a missing label displays as the domain default `Idle`. An Idle Slot is **reserved** Bay time, not free capacity: its label may stand for real non-Job activity (e.g. "Bay Tidying"), so scheduling never silently consumes or shrinks one — inserting work at a date inside any Slot splits it and pushes the rest of the queue later, the same for Idle and Work Slots alike. Idle Slots can be created automatically when booking work after the queue has gone idle (the gap filled is the count of **working days** between the queue's end and today, never raw calendar days), or manually from a slot context menu (`Add idle slot before` / `Add idle slot after`). Adjacent Idle Slots are valid and are not automatically merged; planners can remove and resize them manually.
...
**Insert at Date**:
The booking move that starts a Slot on a chosen working day by translating that date into a Bay Queue position at the moment of booking. The date is a placement hint, never stored — dates stay derived (see Slot Projection), so later queue or calendar changes move the slot like any other. The pickable range runs from tomorrow to the Bay's next available day: the Slot projected over today is never disturbed, and dates past the queue's end are not offered (deliberate deferral is expressed by placing Idle Slots, not by the date). A date strictly inside an existing Slot splits it — Work or Idle alike — into two Slots of the same Job or label whose working days sum to the original, pushing everything after later; a date on a Slot's projected start inserts cleanly before it; the next available day is a plain append. If the queue shrinks between choosing and booking, the booking clamps to append — the work starts earlier than picked, never later behind machine-made idle. Because it can move existing Slots it carries scheduling authority (`job:schedule`), not just Job-creation authority, and the planner is warned before booking whenever a split will occur, naming the affected Job or Idle label.
_Avoid_: Pinned start, fixed date, scheduled date, due date.
...
**Job Bay Seeding**:
The optional Bay-assignment step of the Create Job form. The form pre-fills editable rows from the Quote Product's enabled Product Bays, silently skipping Product Bays whose Bay is disabled; Products with no enabled Product Bays start with zero selected rows. It lists every enabled Bay grouped by Department in Pipeline order and lets the planner add or remove Bays (including several in one Department) and edit each row's working-days before submitting. Each row carries working-days and a Start Date defaulting to the Bay's next available day; with scheduling authority the planner may pull the date earlier (see Insert at Date), otherwise the row appends as before. Each retained row books one Work Slot into that Bay's queue in the same transaction as Job creation — `durationDays` counting working days, with the same auto-inserted Idle Slot gap that booking on the Gantt uses when a Bay's queue ended before today. The form previews the seeded Slots on the schedule chart for the selected Bays before creation; surrounding-Slot adjustments made there are real schedule edits that apply immediately and survive cancelling the form — cancel discards only the uncreated Job and its previewed Slots. Seeding is optional: a Job may be created with zero Bays and scheduled entirely later on the Gantt.
