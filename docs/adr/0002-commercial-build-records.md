...
Job Document Snapshot copies the Product's current Documents onto a Product Job at creation, pointing at the same immutable stored objects and freezing metadata. Custom Jobs start with an empty Documents collection. Quote Documents are generated customer-facing PDF packets owned by Quotes; the newest created Quote Document is treated as the latest.
