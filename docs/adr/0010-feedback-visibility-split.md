...
Feedback is an internal report a signed-in user submits about one subject (a Quote or a Job). Visibility now splits by Kind. Job `general` Feedback is public within the workspace: anyone who can read the Job can read it (`equipment_job:read`), and anyone who can update the Job can change its Status, including reopening (`equipment_job:update`). Quote `general` Feedback remains private until a quote-scoped read/update surface exists. Corrective Feedback (department or user targeted) and Internal Notes remain exclusive to `super-admin` via `equipment_feedback:read`/`equipment_feedback:update`. This replaces ADR 0008's rule that all Feedback was super-admin-only and fire-and-forget for the submitter; we made job general Feedback public precisely so the team working a Job can see and act on it, while corrective feedback about people stays behind the super-admin wall.

Subject-scoped access rides dedicated endpoints (`feedback.listJobFeedback` gated by `equipment_job:read`, `feedback.updateJobFeedback` gated by `equipment_job:update`) rather than per-role response shaping on the super-admin inbox reads. These endpoints return and mutate only `kind: 'general'` items for every caller — super-admins included — so a response never varies by role and cannot leak corrective items through caches; corrective review stays on the `/feedback` inbox. The quote-side endpoints should follow the same policy when a quote surface needs public general feedback.
...
- **Widen `equipment_feedback:read` to more roles** — rejected; it is all-kinds access, so it cannot express "general only", and the inbox (with Internal Notes and corrective targets) must stay super-admin-exclusive.
