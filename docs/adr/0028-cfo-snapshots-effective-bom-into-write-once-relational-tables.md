...
- Per-assembly content is name + kind only (pricing lives on the Quote, not the fabrication document). Per-part content is `part_id` + `quantity` only; the dump reads through the live Part for display.
